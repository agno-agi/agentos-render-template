...
Deploy a multi-agent system to production on Render.
...
```sh
git clone https://github.com/agno-agi/agentos-render-template.git agentos-render
cd agentos-render
...
1. Open [os.agno.com](https://os.agno.com)
2. Click "Add OS" → "Live"
3. Enter your Render domain (e.g., `https://agentos-api.onrender.com`)
...
render ssh <SERVICE_ID>            # SSH into service
render deploys create <SERVICE_ID> # Trigger redeploy
render psql <DATABASE_ID>          # Connect to database
```

To delete resources:
```sh
render services delete <SERVICE_ID>
render postgres delete <DATABASE_ID>
```
...
Edit `render.yaml` to change the plan or add more instances:
```yaml
services:
  - type: web
    name: agentos-api
